    ...
    @foreach ($storages as $storage)
        <tr class="hover:bg-slate-300 dark:hover:bg-background-secondary transition-colors border border-border">
            ...
            <td class="px-4 py-3 text-right">
                @if($storage->id != $installedStorage->id)
                    <button type="button" data-modal-open="buyModal" data-hw-id="{{ $storage->id }}" data-buy-type="disk" class="inline-flex items-center justify-center rounded-lg border px-3 py-1.5 text-sm font-semibold transition border-cyan-500 text-cyan-600 hover:bg-cyan-50 dark:text-cyan-400 dark:hover:bg-cyan-500/10">
