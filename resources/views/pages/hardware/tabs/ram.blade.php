<table class="w-full text-sm text-left">
    <thead class="text-xs text-text-secondary uppercase bg-background-secondary/50 border-b border-border border-default">
        <tr class="border border-default">
            ...
            <th scope="col" class="px-4 py-2 w-32">Capacity</th>
    ...
    <tbody>
        @foreach($rams as $ram)
            <tr class="hover:bg-slate-300 dark:hover:bg-background-secondary transition-colors border border-border {{ $ram->is_compatible ? 'hover:bg-slate-50 dark:hover:bg-white/5' : 'opacity-50 cursor-not-allowed' }}">
                ...
                <td class="px-4 py-3 text-right">
                    @if($ram->id != $installedRAM->id)
                        <button type="button" data-modal-open="buyModal" data-hw-id="{{ $ram->id }}" data-buy-type="ram" class="inline-flex items-center justify-center rounded-lg border px-3 py-1.5 text-sm font-semibold transition border-cyan-500 text-cyan-600 hover:bg-cyan-50 dark:text-cyan-400 dark:hover:bg-cyan-500/10">
