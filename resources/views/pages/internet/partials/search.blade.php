<form method="GET" action="{{ route('internet.index') }}">
    <div class="mb-6 bg-background-secondary border border-default border-border p-4 flex items-center justify-between">
        <div class="flex w-full items-center gap-3">
            ...
            <input type="text" name="ip" placeholder="1.2.3.4" value="{{ old('ip', request('ip', isset($network) ? $network->ip : '')) }}" class="w-full bg-background-primary border border-default border-border px-3 py-2 text-sm outline-none text-accent-primary">
